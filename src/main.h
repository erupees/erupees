...
// Copyright (c) 2011-2017 The Erupees developers
...
public:
    ...
    {
        // erpcoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // erpcoin: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // erpcoin: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // erpcoin: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // erpcoin: whether transaction is a coinstake
    bool fCoinStake;

    // erpcoin: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // erpcoin: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // erpcoin
            undo.nTime = nTime;            // erpcoin
        }
        return true;
    }
...
public:
    ...
    // erpcoin: block signature - signed by coin base txout[0]'s owner
    ...
    // erpcoin: two types of block: proof-of-work or proof-of-stake
    ...
    // erpcoin: get max transaction timestamp
    ...
    bool GetCoinAge(uint64& nCoinAge) const; // erpcoin: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // erpcoin: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (erpcoin proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // erpcoin: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // erpcoin: proof-of-stake related block index fields
    unsigned int nFlags;  // erpcoin: block index flags
    enum
    {
