...
// erpcoin: optional setting to unlock wallet for block minting only;
...
int64 CWalletTx::GetTxTime() const
{
    ...
    //erpcoin: we still have the timestamp, so use it to avoid confusion
    return nTime;
}
...
// erpcoin: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->nTime > nSpendTime)
                continue;  // erpcoin: timestamp must not exceed spend time
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            loop
            {
                ...
                // erpcoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else if (!GetBoolArg("-avatar")) // erpcoin: not avatar mode; nu: avatar mode enabled by default only on Share wallet to avoid change being sent to hidden address
                    {
...
// erpcoin: create coin stake transaction
...
// erpcoin: check 'spent' consistency between wallet and txindex
// erpcoin: fix wallet spent state according to txindex
...
// erpcoin: disable transaction (only for coinstake)
