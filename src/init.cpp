...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("Erupees version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  erupeesd [options]                     " + "\n" +
                  "  erupeesd [options] <command> [params]  " + _("Send command to -server or erupeesd") + "\n" +
                  "  erupeesd [options] help                " + _("List commands") + "\n" +
                  "  erupeesd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "erupees:") && !boost::algorithm::istarts_with(argv[i], "erpcoin:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: erupees.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: erupeesd.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    //
    // erpcoin: -mintxfee and -minrelaytxfee options of bitcoin disabled
    ...
    if (mapArgs.count("-checkpointkey")) // erpcoin: checkpoint master priv key
    {
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Erupees is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Erupees version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "Erupees server starting\n");
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Erupees") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Erupees to complete") << "\n";
