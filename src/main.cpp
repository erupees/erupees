...
// Copyright (c) 2011-2015 The Erupees developers
...
const string strMessageMagic = "Erupees Signed Message:\n";
...
void SyncWithWallets(const uint256 &hash, const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // erpcoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        // erpcoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // erpcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    // erpcoin: subsidy is cut in half every 16x multiply of difficulty
...
// erpcoin: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// erpcoin: find last block index up to pindex
...
#endif
{
    ...
    // erpcoin: target change every block
    // erpcoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // erpcoin: should not enter safe mode for longer invalid chain
}
...
bool CTransaction::CheckInputs(CValidationState &state, CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, std::vector<CScriptCheck> *pvChecks) const
{
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // erpcoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // erpcoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // erpcoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view, bool *pfClean)
{
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (undo.nHeight != 0) {
                    ...
                    coins.fCoinStake = undo.fCoinStake; // erpcoin
                    coins.nTime = undo.nTime;           // erpcoin
                } else {
                    if (coins.IsPruned())
    ...
    // erpcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // erpcoin: coinbase reward check relocated to CheckBlock()
    ...
    // erpcoin: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;

    // erpcoin: fees are not collected by miners as in bitcoin
    // erpcoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// erpcoin: total coin age spent in transaction, in the unit of coin-days.
...
// erpcoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(CValidationState &state, const CDiskBlockPos &pos)
{
    ...
    // erpcoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // erpcoin: record proof-of-stake hash value
    ...
    // erpcoin: compute stake modifier
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // erpcoin: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return state.DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // erpcoin: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // erpcoin: check that the block satisfies synchronized checkpoint
    ...
    // erpcoin: check pending sync-checkpoint
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    // erpcoin: verify hash target and signature of coinstake tx
    if (pblock->IsProofOfStake())
    {
    ...
    // erpcoin: ask for pending sync-checkpoint if any
    ...
    if (pblock->hashPrevBlock != 0 && !mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // erpcoin: getblocks may not obtain the ancestor block rejected
    ...
    // erpcoin: if responsible for sync-checkpoint send it
...
// erpcoin: entropy bit for stake modifier if chosen by modifier
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(int, CBlockIndex*)& item, vSortedByHeight)
    {
        ...
        // erpcoin: calculate stake modifier checksum
    ...
    // erpcoin: load hashSyncCheckpoint
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Erupees", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
bool InitBlockIndex() {
    ...
    fTxIndex = true; // erpcoin: txindex is always enabled
    ...
    if (!fReindex) {
        ...
        // erpcoin: initialize synchronized checkpoint
        if (!WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // erpcoin: if checkpoint master key changed must reset sync-checkpoint
...
string GetWarnings(string strFor)
{
    ...
    // erpcoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // erpcoin: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // erpcoin: safe mode for high alert
            }
        }
    }
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            // erpcoin: relay memory may contain blocks too
            bool found = false;
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // erpcoin: send latest proof-of-work block to allow the
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // erpcoin: record my external IP reported by peer
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // erpcoin: tell downloading node about the latest block if it's
...
CBlockTemplate* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // erpcoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // erpcoin: simplify transaction fee - allow free = false
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("ErupeesMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ErupeesMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ErupeesMiner : ProcessBlock, block not accepted");
    }
...
#endif
{
    ...
    RenameThread(fProofOfStake? "erpcoin-stake-minter" : "erpcoin-miner");
    ...
    try { loop {
        ...
        if (fProofOfStake)
        {
            // erpcoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
        ...
        printf("Running ErupeesMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("ErupeesMiner terminated\n");
        throw;
    }
}
...
// erpcoin: stake minter
void MintStake(boost::thread_group& threadGroup, CWallet* pwallet)
{
    // erpcoin: mint proof-of-stake blocks in the background
